...
- ทดสอบ free tier ได้ด้วย `CBOE/VIX` ก่อน
        ...
        if date_mode == "📅 วันเดียว":
            ...
            st.session_state["hist_dates"]   = [as_of.strftime("%Y-%m-%d")]
            st.session_state["hist_datemode"] = "single"
            ...
            all_dates = [d.strftime("%Y-%m-%d") for d in dates_rng]
            st.session_state["hist_dates"]    = all_dates
            st.session_state["hist_datemode"] = "range"
            st.info(f"จะโหลด **{len(all_dates)} วัน**")

        st.session_state["hist_apikey_val"] = api_key
        st.session_state["hist_ds_code"]    = ds_info["code"]
        st.session_state["hist_ds_label"]   = ds_label
    ...
    def render_main(self) -> None:
        sym = st.session_state.get("hist_sym", "SPY")
        st.subheader(f"📅 ข้อมูลย้อนหลัง — {sym}")

        if not st.session_state.get("hist_apikey_val"):
            st.info("👈 ใส่ API Key และตั้งค่าใน Sidebar"); return

        if st.session_state.get("hist_run"):
            api_key  = st.session_state["hist_apikey_val"]
            ds_code  = st.session_state["hist_ds_code"]
            dates    = st.session_state.get("hist_dates", [])
