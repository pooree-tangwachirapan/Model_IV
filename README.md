...
ส่ง **2 รอบต่อวันทำการ**:

| รอบ | เวลา ET | เวลาไทย | ใช้ทำอะไร |
|---|---|---|---|
| ก่อนเปิดตลาด | **09:00** (ระฆังเปิด − 30 นาที) | 20:00 | เตรียมแผนก่อนเข้าตลาด |
| เปิดแล้ว 30 นาที | **10:00** | 21:00 | ดู opening range ของจริง |

หัวเรื่องเมลติดป้ายบอกรอบไว้ เช่น `[ก่อนเปิดตลาด 30 นาที]` แยกกันได้ในกล่องจดหมาย

### ⏰ ทำไมเวลาถึงตรง ทั้งที่ GitHub cron ไม่ตรง

GitHub ยิง scheduled workflow สายได้เป็นชั่วโมง และ **สายไม่คงที่** —
วัดจาก run จริงของ repo นี้: ส.ค. สาย 38–48 นาที · ก.ย. สาย 221–324 นาที
เมลรอบเช้าเลยเคยมาถึงตอนบ่ายสองของ ET ติดกันหนึ่งเดือนเต็มโดยที่ทุก run ขึ้น success

ตอนนี้ workflow **ไม่เชื่อเวลาที่ GitHub เริ่มงานอีกแล้ว**:
1. cron ตั้งเป็นบันไดหลายชั้น ครอบดีเลย์ 0–6 ชม.
2. ชั้นที่ได้วิ่งจะคำนวณเวลาเป้าเองจาก `market_clock.py` แล้วนอนรอจนถึงเวลานั้น
3. ชั้นที่มาสายเกินหรือเช้าเกินก็ออกเอง ไม่ส่งซ้ำ ไม่เผารันเนอร์

`market_clock.py` รู้จัก DST และวันหยุดตลาด (คำนวณจากกฎ ไม่ใช่ตารางที่ต้องเติมทุกปี)
จึงไม่ต้องแก้ cron ตอนเปลี่ยนฤดู และไม่ส่งเมลในวันหยุด

```
python market_clock.py info        # วันนี้ตลาดเปิดไหม · แต่ละ slot กี่โมง UTC
```
...
กดรันเองได้ที่แท็บ **Actions → Daily Signal Recap → Run workflow**
· ติ๊ก **now** เพื่อส่งทันทีโดยไม่ต้องรอเวลาเป้า (ใช้ทดสอบ)
· ช่อง **slots** ใส่ `premarket` หรือ `open30` เพื่อส่งเฉพาะรอบนั้น
...
## 🗃️ Tab 0DTE — เก็บ option 0DTE ทุก strike ไว้วิเคราะห์ย้อนหลัง

CBOE ให้ **ภาพปัจจุบันเท่านั้น ไม่มีย้อนหลัง** ถ้าไม่เก็บเอง ก็ถามย้อนหลังไม่ได้เลย —
คำถามอย่าง "ตอน 10:00 คนซื้อ call ที่ strike ไหน แล้วราคาไปทางนั้นจริงไหม" ตอบไม่ได้

แท็บนี้เก็บ chain ของ expiry วันนี้ **ทุก strike ทุก 15 นาที** ลง CSV:

```
zero_dte/QQQ/2026-09-25.csv       วันที่ยังใช้งาน (Excel เปิดได้ตรง ๆ)
zero_dte/QQQ/2026-09.csv.gz       เดือนที่จบแล้ว (รวบอัตโนมัติวันที่ 1)
```

หนึ่งแถว = สัญญาหนึ่งตัว ณ เวลาหนึ่ง · มี `bid/ask/mid/last/volume/OI/IV` +
greeks ครบ (`delta/gamma/vega/theta`) + OHLC ของวัน

**เก็บอัตโนมัติ** โดย workflow `armed-alert` ทุก 15 นาทีตอนตลาดเปิด
· กดปุ่ม **📥 บันทึก snapshot เดี๋ยวนี้** ในแท็บเพื่อเก็บจุดเพิ่มนอกตาราง

### 📊 ปุ่มออกรายงาน HTML

กด **📊 สร้างรายงาน** แล้วดาวน์โหลดเป็นไฟล์ HTML ไฟล์เดียว เปิดที่ไหนก็ได้ มี 6 กราฟ:

| # | กราฟ | ตอบคำถามอะไร |
|---|---|---|
| 1 | ราคา underlying ตลอดวัน | บริบท — ดูอย่างอื่นไม่รู้เรื่องถ้าไม่มีอันนี้ |
| 2 | ปริมาณต่อ strike (call/put) | คนเล่นที่ไหน |
| 3 | **พรีเมียมที่เปลี่ยนมือต่อ strike** | เงินไปไหน — ต่างจากข้อ 2 เพราะ 1,000 สัญญาที่ $0.05 ไม่เท่า 100 สัญญาที่ $5 |
| 4 | ราคา mid ของสัญญาที่คึกที่สุด | กราฟราคาของตัว option เอง |
| 5 | heatmap ปริมาณ (strike × เวลา) | flow ย้ายฐานตอนไหน |
| 6 | Put/Call ratio ตามเวลา | regime ภายในวัน |

พร้อมตารางต่อ strike (OHLC ของวัน · volume · OI · IV · greeks)

จากบรรทัดคำสั่งก็ได้:
```
python zero_dte_cli.py record                     # เก็บเดี๋ยวนี้
python zero_dte_cli.py summary                    # สรุปวันล่าสุด
python zero_dte_cli.py report --day 2026-09-25    # ออกรายงาน HTML
```

⚠️ **นี่คือเครื่องเก็บข้อมูล ไม่ใช่ระบบสัญญาณ** — ไม่มีคำแนะนำซื้อขายในแท็บนี้
และยังไม่ควรเอาไปตั้ง threshold อะไรจนกว่าจะเก็บได้พอ
· `volume` ที่ CBOE ส่งมาเป็นยอดสะสม รายงานแปลงเป็นปริมาณต่อช่วงให้แล้ว
· CBOE ดีเลย์ ~15 นาที ค่าที่ติดป้าย 10:00 คือภาพราว 09:45
