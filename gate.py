...
def evaluate(snap: dict) -> dict:
    ...
    if em:
        # วัด "ความกว้างโซน" เทียบ ±1σ ไม่ใช่ "ระยะถึงกำแพงที่ใกล้ที่สุด" —
        # เพราะ ARMED แปลว่าราคาชิดกำแพงอยู่แล้ว ระยะนั้นจึงต้องน้อยเสมอ เช็คแบบนั้นจะเตือนทุกครั้งโดยไม่มีความหมาย
        Sf.append(_g("ความกว้างโซนเทียบ EM", z["span"] >= 2 * em,
                     f"โซนกว้าง {z['span']:,.2f} · EM 1σ ±{em:,.2f}",
                     "โซนแคบกว่า ±1σ = ราคาคาดว่าจะกวาดทั้งโซนได้ในวันเดียว "
                     "กำแพงทั้งสองฝั่งจะโดนทดสอบ สมมติฐาน 'ถูกดูดกลับ' อ่อนลง"))
